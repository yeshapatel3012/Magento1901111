/* Login popup class */
var MagecomOneStepCheckoutLoginPopup = Class.create({
    initialize: function(options) {
        this.options = options;
        this.popup_container = $('magecomonestepcheckout-login-popup');
        this.popup_link = $('magecomonestepcheckout-login-link');
        this.popup = null;
        this.createPopup();
        this.mode = 'login';

        this.forgot_password_link = $('magecomonestepcheckout-forgot-password-link');
        this.forgot_password_container = $('magecomonestepcheckout-login-popup-contents-forgot');
        this.forgot_password_loading = $('magecomonestepcheckout-forgot-loading');
        this.forgot_password_error = $('magecomonestepcheckout-forgot-error');
        this.forgot_password_success = $('magecomonestepcheckout-forgot-success');
        this.forgot_password_button = $('magecomonestepcheckout-forgot-button');
        this.forgot_password_table = $('magecomonestepcheckout-forgot-table');

        this.login_link = $('magecomonestepcheckout-return-login-link');
        this.login_container = $('magecomonestepcheckout-login-popup-contents-login');
        this.login_table = $('magecomonestepcheckout-login-table');
        this.login_error = $('magecomonestepcheckout-login-error');
        this.login_loading = $('magecomonestepcheckout-login-loading');
        this.login_button = $('magecomonestepcheckout-login-button');
        this.login_form = $('magecomonestepcheckout-login-form');
        this.login_username = $('id_magecomonestepcheckout_username');
        ...
        this.forgot_password_handler = function(e) {
            var email = $('id_magecomonestepcheckout_email').getValue();
...
function get_totals_element()
{
    ...
    var search_osc = $$('div.magecomonestepcheckout-summary');
...
function get_save_methods_function(url, update_payments)
{
    ...
    return function(e)    {
        ...
        var form = $('magecomonestepcheckout-form');
            ...
            if(transport.status == 200)    {
                ...
                if(update_payments)    {
                    ...
                    $$('div.payment-methods input[name="payment\[method\]"]').invoke('observe', 'click', function() {
                        $$('div.magecomonestepcheckout-payment-method-error').each(function(item) {
...
function magecomexclude_unchecked_checkboxes(data)
{
    var items = [];
...
function get_save_billing_function(url, set_methods_url, update_payments, triggered)
{
    ...
    return function()    {
        var form = $('magecomonestepcheckout-form');
        var items = magecomexclude_unchecked_checkboxes($$('input[name^=billing]').concat($$('select[name^=billing]')));
        var names = items.pluck('name');
        var values = items.pluck('value');
        var parameters = {
        ...
        var shipment_methods = $$('div.magecomonestepcheckout-shipping-method-block')[0];
            ...
            if(transport.status == 200)    {
                if(shipment_methods_found)  {
                    ...
                    $$('dl.shipment-methods input').invoke('observe', 'click', function() {
                        $$('div.magecomonestepcheckout-shipment-method-error').each(function(item) {
                            new Effect.Fade(item);
                        });
                    });
                }
        ...
        new Ajax.Request(url, {
            ...
            if(transport.status == 200)    {
                ...
                if(update_payments)    {
                    ...
                    if($RF($('magecomonestepcheckout-form'), 'payment[method]') != null)    {
                        try    {
                            var payment_method = $RF(form, 'payment[method]');
                            $('container_payment_method_' + payment_method).show();
                            $('payment_form_' + payment_method).show();
...
function addressPreview(templates, target) {
    ...
    if(targetelem){
        targetelem.update('');
        targetelem.insert($$('table.magecomonestepcheckout-summary')[0].cloneNode(true));
        targetelem.insert($$('table.magecomonestepcheckout-totals')[0].cloneNode(true));
    }
}
