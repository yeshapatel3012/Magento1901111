<?php
class Ys_MagecomOneStepCheckout_Helper_Data extends Mage_Core_Helper_Abstract
{
    public function checkLicValid($observer)
    {
        $layout = Mage::app()->getLayout();
        $content = $layout->getBlock('content');
        $block = $layout->createBlock('magecomonestepcheckout/magecomvalid');
        $content->insert($block);
    }

    public function setCustomerComment($observer)
    {
        $enableComments = Mage::getStoreConfig('magecomonestepcheckout/exclude_fields/enable_comments');
        $enableCommentsDefault = Mage::getStoreConfig('magecomonestepcheckout/exclude_fields/enable_comments_default');
        $enableFeedback = Mage::getStoreConfig('magecomonestepcheckout/feedback/enable_feedback');
        $orderComment = $this->_getRequest()->getPost('magecomonestepcheckout_comments');
        ...
        if($enableFeedback){
            ...
            $feedbackValues = unserialize(Mage::getStoreConfig('magecomonestepcheckout/feedback/feedback_values'));
            $feedbackValue = $this->_getRequest()->getPost('magecomonestepcheckout-feedback');
            $feedbackValueFreetext = $this->_getRequest()->getPost('magecomonestepcheckout-feedback-freetext');
    ...
    {
        return Mage::getStoreConfig('magecomonestepcheckout/general/rewrite_checkout_links');
    }
