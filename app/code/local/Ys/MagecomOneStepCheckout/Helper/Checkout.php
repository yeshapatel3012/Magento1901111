<?php
class Ys_MagecomOneStepCheckout_Helper_Checkout extends Mage_Core_Helper_Abstract
{
    ...
    function __construct()
    {
        ...
        $this->temp = Mage::getStoreConfig('magecomonestepcheckout/general/serial');
    }
    ...
    public function loadConfig()
    {
        ...
        foreach($items as $config)    {
            ...
            $settings[$name] = Mage::getStoreConfig('magecomonestepcheckout/' . $config);
        }
   ...
   function checkMagecomEntry($magedom, $mageser)
    { 
         if(sha1(sha1(sha1(base64_decode('bWFnZWNvbW9uZXN0ZXBjaGVja291dA==')).$magedom)) == $mageser)   {
            return true;
        }
    ...
    function checkMagecomEntryDev($magedom, $mageser)
    {
         if(sha1(sha1(sha1(base64_decode('bWFnZWNvbW9uZXN0ZXBjaGVja291dF9kZXY=')).$magedom)) == $mageser)   {
            return true;
        }
    ...
    public function canRun($dev=false)
    {
        ...
        if(!$dev) {
            $original = $this->checkMagecomEntry($_SERVER['SERVER_NAME'], $temp);
            $wildcard = $this->checkMagecomEntry($this->getDomain(), $temp);
        } else {
            $original = $this->checkMagecomEntryDev($_SERVER['SERVER_NAME'], $temp);
            $wildcard = $this->checkMagecomEntryDev($this->getDomain(), $temp);
        }
