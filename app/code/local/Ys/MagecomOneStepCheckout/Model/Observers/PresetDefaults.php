<?php
class Ys_MagecomOneStepCheckout_Model_Observers_PresetDefaults extends Mage_Core_Model_Abstract {
    ...
    public function setDefaults(Varien_Event_Observer $observer) {
        ...
        if(Mage::getStoreConfig('magecomonestepcheckout/general/rewrite_checkout_links', $quote->getStore())) {
    ...
    public function setDefaultsOnLogin(Varien_Event_Observer $observer) {
        ...
        if(is_object($quote)){
            ...
            if(Mage::getStoreConfig('magecomonestepcheckout/general/rewrite_checkout_links')) {
                $this->callDefaults($observer);
            }
        }
    ...
    public function setShippingIfDifferent(Varien_Event_Observer $observer){
        ...
        $newCode = Mage::getStoreConfig('magecomonestepcheckout/general/default_shipping_method', $quote->getStore());
    ...
    public function setAddressDefaults(Varien_Event_Observer $observer) {
        ...
        if (empty($currentShipping) && ! empty($newShipping)) {
            ...
            $quote->getShippingAddress()->setSameAsBilling(Mage::getStoreConfig('magecomonestepcheckout/general/enable_different_shipping_hide', $quote->getStore()));
        }
    ...
    public function getCheckPostcode (){
        return ((strstr(Mage::getStoreConfig('magecomonestepcheckout/ajax_update/ajax_save_billing_fields'),'postcode')) ? true : false);
    }
    ...
    public function getAddressDefaults(Mage_Sales_Model_Quote $quote){
        ...
        if($countryId = Mage::getStoreConfig('magecomonestepcheckout/general/default_country',$quote->getStore())){
            $data['country_id'] = $countryId;
        }
        if($regionId = Mage::getStoreConfig('magecomonestepcheckout/general/default_region_id',$quote->getStore())){
            $data['region_id'] = $regionId;
        }
        if($city = Mage::getStoreConfig('magecomonestepcheckout/general/default_city',$quote->getStore())){
            $data['city'] = $city;
        }
        if($postcode = Mage::getStoreConfig('magecomonestepcheckout/general/default_postcode',$quote->getStore())){
    ...
    public function getAddressGeoIP(Mage_Sales_Model_Quote $quote){
        ...
        $enabled = Mage::getStoreConfig('magecomonestepcheckout/general/enable_geoip',$quote->getStore());
        $database = Mage::getBaseDir('base') . DS . Mage::getStoreConfig('magecomonestepcheckout/general/geoip_database',$quote->getStore());
        ...
        try {
            if (!@include_once('Net/GeoIP.php')) {
                Mage::throwException(Mage::helper('magecomonestepcheckout')->__('Net/GeoIP pear package is not installed or inaccessible'));
            } else {
        ...
        try {
            ...
            //no database
                ...
                Mage::throwException(Mage::helper('magecomonestepcheckout')->__('Net/GeoIP database %s is not installed properly or is inaccessible', $database));
            }
        } catch (Exception $e) {
            Mage::logException($e);
        }
    ...
    public function setShippingDefaults(Varien_Event_Observer $observer) {
        ...
        if ($codeCount === 1) {
            if(Mage::getStoreConfig('magecomonestepcheckout/general/default_shipping_if_one', $quote->getStore())){
    ...
    public function setPaymentDefaults(Varien_Event_Observer $observer) {
        $quote = $observer->getEvent()->getQuote();
        $newCode = Mage::getStoreConfig('magecomonestepcheckout/general/default_payment_method', $quote->getStore());
